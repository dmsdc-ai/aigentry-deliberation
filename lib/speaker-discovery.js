...
import { execFileSyncCliCommand } from "./cli-process.js";
...
export const CLI_INVOCATION_HINTS = {
  ...
  // Upstream agy (Antigravity) limitation, not our bug: headless --print never flushes stdout
  // here (0 bytes over 180-298s, even with --dangerously-skip-permissions), so cli_auto would
  // hang the whole deliberation. Marked not cli_auto-capable -> runCliAutoTurnCore degrades it to
  // manual. Flip cliAutoCapable back to true once agy fixes headless stdout flushing.
  agy: { cmd: "agy", flags: '-p', example: 'agy -p "prompt"', modelFlag: '--model', defaultModel: null, provider: 'gemini', cliAutoCapable: false },
...
// Task #1172 — selection provenance is an explicit, truthful label on the start
// token. It records WHO composed the speaker set, never that the composition was
// approved, authenticated, or authorized to execute anything.
//   user-selected        a human picked the set in the TUI (deliberation_confirm_speakers)
//   controller-delegated a controller composed the set under a prior delegation
//                        (deliberation_select_speakers); the delegation fields are
//                        an audit CLAIM supplied by the caller, not validated authority
//   legacy-unlabeled     a confirmed token minted before origins existed; accepted for
//                        compatibility only and explicitly NOT a human-authentication claim
export const SELECTION_ORIGIN_USER = "user-selected";
export const SELECTION_ORIGIN_CONTROLLER_DELEGATED = "controller-delegated";
export const SELECTION_ORIGIN_LEGACY_UNLABELED = "legacy-unlabeled";

export const KNOWN_SELECTION_ORIGINS = Object.freeze([
  SELECTION_ORIGIN_USER,
  SELECTION_ORIGIN_CONTROLLER_DELEGATED,
]);

// Bound the audit claim so a delegated token cannot become an arbitrary payload
// carrier in the selection state file.
export const DELEGATION_FIELD_MAX_LENGTH = 200;

// The reference is recorded verbatim as a claim. It is NEVER opened, resolved,
// fetched or otherwise dereferenced — doing so would turn an audit string into an
// implicit authority lookup.
export function normalizeDelegationMetadata(delegation) {
  if (!delegation || typeof delegation !== "object" || Array.isArray(delegation)) {
    return { ok: false, code: "missing_delegation" };
  }
  const normalizeField = (value) => (typeof value === "string" ? value.trim() : "");
  const taskId = normalizeField(delegation.task_id);
  const reference = normalizeField(delegation.reference);
  if (!taskId) return { ok: false, code: "missing_delegation_task_id" };
  if (taskId.length > DELEGATION_FIELD_MAX_LENGTH) {
    return { ok: false, code: "delegation_task_id_too_long", max_length: DELEGATION_FIELD_MAX_LENGTH };
  }
  if (!reference) return { ok: false, code: "missing_delegation_reference" };
  if (reference.length > DELEGATION_FIELD_MAX_LENGTH) {
    return { ok: false, code: "delegation_reference_too_long", max_length: DELEGATION_FIELD_MAX_LENGTH };
  }
  return { ok: true, delegation: { task_id: taskId, reference, claim: true } };
}

// Resolve the provenance of an already-confirmed selection state. Unknown values
// are refused rather than coerced into a human label.
export function resolveSelectionOrigin(selectionState) {
  const raw = selectionState?.selection_origin;
  if (raw === undefined || raw === null || raw === "") {
    return {
      ok: true,
      origin: SELECTION_ORIGIN_LEGACY_UNLABELED,
      legacy: true,
      delegation: null,
    };
  }
  if (typeof raw !== "string" || !KNOWN_SELECTION_ORIGINS.includes(raw)) {
    return { ok: false, code: "unknown_selection_origin", origin: typeof raw === "string" ? raw : null };
  }
  if (raw === SELECTION_ORIGIN_CONTROLLER_DELEGATED) {
    // Task #1172 — the delegation claim is re-validated on READ, against the very
    // same contract `normalizeDelegationMetadata` applies at mint. The stored file
    // is untrusted input: a claim that was never minted through this server (or was
    // edited afterwards) must not become an unbounded payload that rides into the
    // session record, history and archive. Missing / null / array / non-object /
    // blank / oversized refuses here — and this resolver runs BEFORE the token is
    // stamped consumed and before any session is created, so a refusal costs the
    // caller nothing but the retry. Nothing is truncated: an out-of-bound claim is
    // refused outright, never silently shortened into a passing one.
    const storedDelegation = normalizeDelegationMetadata(selectionState.delegation);
    if (!storedDelegation.ok) {
      return {
        ok: false,
        code: "invalid_stored_delegation",
        origin: raw,
        delegation_code: storedDelegation.code,
        ...(storedDelegation.max_length !== undefined ? { max_length: storedDelegation.max_length } : {}),
      };
    }
    return {
      ok: true,
      origin: raw,
      legacy: false,
      // Canonical shape only — `{task_id, reference, claim: true}`. Extra fields the
      // caller (or a tamperer) parked on the stored object are dropped rather than
      // carried forward; `claim: true` restates that this is an unverified audit
      // claim, never authenticated authority.
...
// Task #1172 — delegated controller composition. Same freshness/snapshot/exact-set
// rules as the TUI path (both go through validateSpeakerSelectionSnapshot); the only
// difference is the provenance label and the bounded delegation claim carried with it.
// This is NOT a human approval and must never be presented as one.
export function selectSpeakersByDelegation({ selectionState, selection_token, speakers, includeBrowserSpeakers, delegation, nowMs = Date.now(), persist = true }) {
  const requestedSpeakers = dedupeSpeakers(speakers || []);
  if (requestedSpeakers.length === 0) {
    return { ok: false, code: "missing_speakers" };
  }

  const delegationValidation = normalizeDelegationMetadata(delegation);
  ...
  const delegatedSelection = {
    ...
    include_browser: !!includeBrowserSpeakers,
    candidate_speakers: dedupeSpeakers(selectionState.candidate_speakers || []),
    selected_speakers: requestedSpeakers,
    selection_origin: SELECTION_ORIGIN_CONTROLLER_DELEGATED,
    delegation: delegationValidation.delegation,
  };
  if (persist) {
    _deps.writeJsonFileAtomic(_deps.getSpeakerSelectionFile(), delegatedSelection);
  }
  return { ok: true, selectionState: delegatedSelection };
}
...
export function validateSpeakerSelectionRequest({ selectionState, selection_token, speakers, includeBrowserSpeakers, nowMs = Date.now() }) {
  ...
  // Task #1172 — an origin the server does not recognise is refused outright.
  // It is never assumed to be human. Legacy tokens carrying no origin at all stay
  // acceptable (resolveSelectionOrigin labels them `legacy-unlabeled`); callers
  // read the label via resolveSelectionOrigin so this return shape is unchanged.
  const originResolution = resolveSelectionOrigin(selectionState);
...
export function checkCliLiveness(command) {
  ...
  try {
    execFileSyncCliCommand(command, ["--version"], {
      stdio: "ignore",
      windowsHide: true,
      timeout: 5000,
      env,
    ...
    try {
      execFileSyncCliCommand(command, ["--help"], {
        stdio: "ignore",
        windowsHide: true,
        timeout: 5000,
        env,
...
// Normalized form of the existing public browser-scan switch
// DELIBERATION_BROWSER_SCAN_MODE ("auto" | "cdp" | "applescript" | "off").
function resolveBrowserScanMode() {
  return (process.env.DELIBERATION_BROWSER_SCAN_MODE || "auto").trim().toLowerCase();
}

export async function ensureCdpAvailable() {
  // Honor an explicit "off" before resolving endpoints, probing the network,
  // touching the Chrome profile, spawning a browser or waiting on it: when the
  // operator has disabled browser scanning, activating CDP is not ours to do.
  if (resolveBrowserScanMode() === "off") {
    return {
      available: false,
      reason: "Browser scanning is disabled (DELIBERATION_BROWSER_SCAN_MODE=off); Chrome CDP activation skipped.",
    };
  }
...
export async function collectBrowserLlmTabs() {
  const mode = resolveBrowserScanMode();
  const tabs = [];
  const notes = [];
...
export async function collectSpeakerCandidates({ include_cli = true, include_browser = true } = {}) {
  ...
  if (include_browser) {
    ...
    // Explicit off means disabled: skip endpoint resolution and probing entirely,
    // so no CDP request leaves this process. An empty set leaves the matching
    // passes below as no-ops without touching injected tabs or include_browser.
    const cdpEndpoints = resolveBrowserScanMode() === "off" ? [] : resolveCdpEndpoints();
    ...
    // transport. The browser adapter will create/navigate tabs on demand if needed.
    if (cdpReachable) {
      for (const candidate of candidates) {
